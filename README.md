...
Animations made in the editor are saved to `config/anima/` as plain JSON. You can edit them by hand, share them, and they override a resource pack definition with the same id.

## In-game editor

Press `F7` (rebindable). If you are already in a world, the editor opens as a floating window and the world keeps rendering, so you can tune things while looking at them. If you are not in a world, it first takes you to a config world.

The config world `anima-config` is a flat spectator world with a fixed time, created automatically. It does not touch your saves. The 3D preview inside the editor needs a real world renderer, which is why a world has to be loaded first.

There are two other entry points: the config button for "Anima" in Mod Menu, or `AnimaApi.openAnimationEditor(parent)` in code.

### Animation editor

An animation list on the left and a live preview on the right, used to edit animation definitions that already exist in a resource pack or in the config:

- Frame time, looping, play mode (`loop` / `once` / `pingpong`), frame interpolation, texture width and height
- Add, edit and remove effects by editing the JSON directly, with validation
- Save writes to `config/anima/animations/<id>.json` and overrides a resource pack definition with the same id; Delete/Reset restores the original resource pack definition
- The "Timeline" button jumps to the timeline editor
...
| `AnimaApi.openAnimationEditor(parent)` / `openTimelineEditor(parent)` | Open the editors |
...
编辑器里调好的动画会保存到 `config/anima/`，是普通 JSON，可以手改、可以发给别人，也能覆盖资源包里同 id 的定义。

## 游戏内编辑器

按 `F7` 打开（可以改键）。如果你已经在世界里，编辑器直接以浮窗形式打开，世界继续渲染，可以边看边调；如果不在世界里，它会先带你去一个配置世界。

配置世界 `anima-config` 是自动创建的超平坦旁观世界，时间固定，不会影响你的存档。编辑器里的 3D 预览需要真实的世界渲染器，所以必须先有世界才能开。

另外还有两个入口：Mod Menu 里点「Anima｜赋灵」的配置按钮，或代码里调用 `AnimaApi.openAnimationEditor(parent)`。

### 动画编辑器

左边是动画列表，右边是实时预览，用来改资源包或配置里已有的动画定义：

- 帧间隔、循环开关、播放模式（`loop` / `once` / `pingpong`）、帧间插值、纹理宽高
- 特效的增删改，直接编辑 JSON 并有校验
- 保存会写入 `config/anima/animations/<id>.json`，覆盖资源包里同 id 的定义；删除/重置则恢复到资源包原定义
- 「编排/时间线」按钮跳到时间线编辑器
...
| `AnimaApi.openAnimationEditor(parent)` / `openTimelineEditor(parent)` | 打开编辑器 |
